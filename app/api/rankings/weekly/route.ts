...
export async function POST(request:Request){try{
 ...
 const {data:run,error}=await supabase.from("game_runs").select("id,user_id,era,score,league_position,league_points,wins,draws,losses,goals_for,goals_against,clean_sheets,trophy_count,manager_id,verification_status").eq("id",runId).eq("user_id",user.id).single();
