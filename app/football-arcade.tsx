...
type LeagueSeason = {league:string;year:number;powerhouses:string[];story:string};
type Result = { title:string; score:number; grade:string; story:string; stats:{label:string;value:string}[]; badges:string[]; detail:string[]; matches?:MatchResult[]; table?:{club:string;played:number;points:number;highlight?:boolean}[]; awards?:string[]; review?:{label:string;value:string;copy:string}[] };
...
const COACHES:Coach[]=[
  {name:"José Mourinho",era:"2004–15",attack:"B",defence:"S",pressing:"A",tag:"DEFENSIVE GURU",attackMod:-.02,defenceMod:.09,cleanSheetMod:.18,quote:"Control the space, control the result."},
  {name:"Pep Guardiola",era:"2008–NOW",attack:"S",defence:"A",pressing:"S",tag:"POSITIONAL MASTER",attackMod:.09,defenceMod:.04,cleanSheetMod:.08,quote:"The ball is the first defender."},
  {name:"Sir Alex Ferguson",era:"1986–13",attack:"S",defence:"A",pressing:"A",tag:"SERIAL WINNER",attackMod:.08,defenceMod:.04,cleanSheetMod:.06,quote:"Win the moment after the 90th minute."},
  {name:"Carlo Ancelotti",era:"1995–NOW",attack:"A",defence:"A",pressing:"B",tag:"PLAYER WHISPERER",attackMod:.05,defenceMod:.05,cleanSheetMod:.07,quote:"Give great players the space to solve games."},
  {name:"Jürgen Klopp",era:"2008–24",attack:"S",defence:"B",pressing:"S",tag:"PRESSING ENGINE",attackMod:.1,defenceMod:.01,cleanSheetMod:.02,quote:"Turn chaos into the best playmaker."},
  {name:"Diego Simeone",era:"2011–NOW",attack:"C",defence:"S",pressing:"A",tag:"SIEGE ARCHITECT",attackMod:-.04,defenceMod:.11,cleanSheetMod:.2,quote:"Suffer together, strike together."},
];
const SEASONS:Record<string,LeagueSeason[]>={
  "80s":[{league:"Serie A",year:1988,powerhouses:["Inter","Milan","Napoli","Juventus","Roma"],story:"Maradona's Napoli and the great Milan machine define the hardest league in Europe."},{league:"English First Division",year:1986,powerhouses:["Liverpool","Everton","Arsenal","Manchester United","Tottenham"],story:"Merseyside rules a direct, unforgiving English landscape."}],
  "90s":[{league:"Serie A",year:1998,powerhouses:["Juventus","Inter","Lazio","Milan","Parma"],story:"Italian football is a weekly tactical examination with no soft fixtures."},{league:"Premier League",year:1999,powerhouses:["Manchester United","Arsenal","Chelsea","Leeds","Liverpool"],story:"United chase history while Wenger's Arsenal modernise England."}],
  "00s":[{league:"La Liga",year:2006,powerhouses:["Barcelona","Real Madrid","Valencia","Sevilla","Villarreal"],story:"Ronaldinho's Barcelona meets Madrid's stars and Valencia's structure."},{league:"Premier League",year:2005,powerhouses:["Chelsea","Manchester United","Arsenal","Liverpool","Tottenham"],story:"Chelsea set a defensive standard while the old powers regroup."}],
  "10s":[{league:"Premier League",year:2015,powerhouses:["Chelsea","Manchester City","Arsenal","Manchester United","Liverpool"],story:"Chelsea are the benchmark; City are loaded, while Liverpool remain unfinished."},{league:"La Liga",year:2017,powerhouses:["Real Madrid","Barcelona","Atlético","Sevilla","Villarreal"],story:"The Clásico giants are at full force and Atlético make every point hurt."}],
  "20s":[{league:"Premier League",year:2024,powerhouses:["Manchester City","Arsenal","Liverpool","Chelsea","Newcastle"],story:"City's machine meets Arsenal's rise and a league built on pressing data."},{league:"Bundesliga",year:2024,powerhouses:["Leverkusen","Bayern","Dortmund","Leipzig","Stuttgart"],story:"Leverkusen break the old order while Bayern chase from unfamiliar ground."}],
};
...
function simulate(game:Game,picks:Pick[],seed:string,league:string,position:Pos,eraId="",coach?:Coach,season?:LeagueSeason):Result{
  ...
  if(game==="era"){
    ...
    const games=38,rivals=season?.powerhouses||LEAGUE_CLUBS[league],matches:MatchResult[]=[];let wins=0,draws=0,losses=0,cleanSheets=0,goalsFor=0,goalsAgainst=0;
    const strength=(score-72)/28,attackBoost=coach?.attackMod||0,defenceBoost=coach?.defenceMod||0;
    for(let i=0;i<games;i++){const opponent=rivals[i%rivals.length],roll=r(),winLine=.53+strength*.27+attackBoost+defenceBoost*.35,drawLine=.77+strength*.08+defenceBoost*.4;let result:"W"|"D"|"L",gf:number,ga:number;
    ...
    const competitorPoints=[pts,...rivals.map((_,i)=>Math.max(55,Math.min(110,Math.round(91-i*5+r()*10))))];const table=["Era XI",...rivals].map((club,i)=>({club,played:games,points:competitorPoints[i],highlight:i===0})).sort((a,b)=>b.points-a.points);
  ...
  if(game==="perfect"){
    ...
    const yourClub="Arcade Five";const competitorPoints=[pts,...rivals.map((_,i)=>Math.max(24,Math.min(games*3,Math.round(48-i*4+r()*6))))];const table=[yourClub,...rivals.map(x=>`${x} Five`)].map((club,i)=>({club,played:games,points:competitorPoints[i],highlight:i===0})).sort((a,b)=>b.points-a.points);
...
function EraSetupPanel({slots,picks,onSwap,coach,season,spinning}:{slots:string[];picks:Pick[];onSwap:(a:number,b:number)=>void;coach:Coach|null;season:LeagueSeason|null;spinning:"coach"|"season"|null}){return <div className="era-final-setup"><div className="final-pitch"><Pitch slots={slots} picks={picks} current={-1} onSwap={onSwap}/></div><div className="final-rolls"><section className={`roll-card ${spinning==="coach"?"rolling":""}`}><span>STEP 04 · MANAGER</span>{coach?<><div className="coach-name"><i>{coach.name.split(" ").map(n=>n[0]).slice(0,2).join("")}</i><div><h3>{coach.name}</h3><b>{coach.tag}</b></div></div><div className="coach-stats"><span>ATTACK <b>{coach.attack}</b></span><span>DEFENCE <b>{coach.defence}</b></span><span>PRESSING <b>{coach.pressing}</b></span></div><p>“{coach.quote}”</p></>:<div className="roll-placeholder">{spinning==="coach"?"SEARCHING THE TOUCHLINE…":"Manager waiting"}</div>}</section><section className={`roll-card ${spinning==="season"?"rolling":""}`}><span>STEP 05 · DESTINATION</span>{season?<><h3>{season.league} · {season.year}</h3><b className="season-tag">HISTORICAL POWER INDEX</b><p>{season.story}</p><div className="powerhouse-list">{season.powerhouses.map((club,i)=><span key={club}><b>{i+1}</b>{club}</span>)}</div></>:<div className="roll-placeholder">{spinning==="season"?"OPENING THE HISTORY BOOK…":"League and year waiting"}</div>}</section></div></div>}
function MatchTimeline({matches}:{matches:MatchResult[]}){return <section className="season-run"><div className="result-section-title"><span>THE FULL CAMPAIGN</span><small>Hover or focus any result for the match report</small></div><div className="match-dots">{matches.map((m,i)=><button style={{animationDelay:`${i*35}ms`}} className={m.result.toLowerCase()} aria-label={`Match ${i+1}, ${m.opponent}, ${m.score}`} key={i}><span>{m.result}</span><div className="match-popover"><small>MATCH {String(i+1).padStart(2,"0")} · {m.tone}</small><h4>{m.opponent}</h4><strong>{m.score}</strong><p>{m.summary}</p></div></button>)}</div></section>}
...
function GameView({game,go}:{game:Game;go:(m:Mode)=>void}){
  const [daily,setDaily]=useState(true),[nonce,setNonce]=useState(0),[picks,setPicks]=useState<Pick[]>([]),[result,setResult]=useState<Result|null>(null),[league,setLeague]=useState(leagues[0]),[position,setPosition]=useState<Pos>("ATT"),[formation,setFormation]=useState<typeof FORMATIONS[number]|null>(null),[selectedEra,setSelectedEra]=useState(""),[rerollUsed,setRerollUsed]=useState(false),[rerollNonce,setRerollNonce]=useState(0),[spinning,setSpinning]=useState(false),[coach,setCoach]=useState<Coach|null>(null),[season,setSeason]=useState<LeagueSeason|null>(null),[phaseSpin,setPhaseSpin]=useState<"coach"|"season"|null>(null);
  ...
  function resetForChoice(){setPicks([]);setResult(null);setFormation(null);setSelectedEra("");setRerollUsed(false);setRerollNonce(0);setCoach(null);setSeason(null);setNonce(n=>n+1)}
  function pick(p:Pick){setPicks(x=>[...x,{...p,slot:slots[round]}])}
  function run(){const res=simulate(game,picks,seed,league,position,selectedEra,coach||undefined,season||undefined);setResult(res);try{localStorage.setItem("football-arcade-last",JSON.stringify({game,picks,seed,league,position,res}))}catch{}}
  function restart(){setPicks([]);setResult(null);setFormation(null);setSelectedEra("");setRerollUsed(false);setRerollNonce(0);setCoach(null);setSeason(null);setNonce(n=>n+1)}
  ...
  function rollCoach(){setPhaseSpin("coach");setTimeout(()=>{setCoach(sample(COACHES,1,seed+"coach")[0]);setPhaseSpin(null)},900)}
  function rollSeason(){setPhaseSpin("season");setTimeout(()=>{const rolled=sample(SEASONS[selectedEra],1,seed+"season")[0];setSeason(rolled);setLeague(rolled.league);setPhaseSpin(null)},900)}
