...
const ERA_SLOTS = ["GK","RB","RCB","LCB","LB","DM","CM","AM","RW","ST","LW"];
const SLOT_POS:Record<string,Pos>={GK:"GK",RB:"DEF",RCB:"DEF",LCB:"DEF",LB:"DEF",DM:"MID",CM:"MID",AM:"MID",RW:"ATT",ST:"ATT",LW:"ATT"};
...
function overall(p:Player){return Math.round(p.attrs.reduce((a,b)=>a+b,0)/6)}
...
function Card({p,onPick,attribute}:{p:Pick;onPick?:()=>void;attribute?:string}){const val=attribute? p.attrs[A.indexOf(attribute)]:overall(p);return <button className="player-card" onClick={onPick} disabled={!onPick} style={{"--accent":p.color} as React.CSSProperties}><div className="card-top"><span>{attribute||p.pos}</span><b>{val}</b></div><div className="portrait"><span>{p.name.split(" ").map(n=>n[0]).slice(0,2).join("")}</span></div><div className="card-copy"><small>{p.era} · {p.nation}</small><h3>{p.name}</h3><p>{p.club} · {p.league}</p></div>{attribute&&<div className="attr-line"><strong>{attribute}</strong><span style={{width:`${val}%`}}/></div>}</button>}
function GameView({game,go}:{game:Game;go:(m:Mode)=>void}){
  const [daily,setDaily]=useState(true),[nonce,setNonce]=useState(0),[picks,setPicks]=useState<Pick[]>([]),[result,setResult]=useState<Result|null>(null),[league,setLeague]=useState(leagues[0]),[position,setPosition]=useState<Pos>("ATT");
  const seed=`${daily?dailySeed():"random-run"}-${game}-${nonce}-${league}-${position}`;
  const slots=game==="era"?ERA_SLOTS:game==="perfect"?PERFECT_SLOTS:A; const round=picks.length;const done=round>=slots.length;
  const needed:Pos|undefined=game==="era"?SLOT_POS[slots[round]]:game==="perfect"?(round===0?"GK":round===1?"DEF":round===2?"MID":round===3?"ATT":undefined):undefined;
  ...
  function resetForChoice(){setPicks([]);setResult(null);setNonce(n=>n+1)}
  ...
  function restart(){setPicks([]);setResult(null);setNonce(n=>n+1)}
  ...
  {!result&&<section className="draft"><div className="progress"><span>ROUND {Math.min(round+1,slots.length)} / {slots.length}</span><div>{slots.map((s,i)=><i className={i<round?"filled":i===round?"now":""} key={s}/>)}</div><b>{done?"SQUAD COMPLETE":game==="player"?`CHOOSE ${A[round]}`:`CHOOSE YOUR ${slots[round]}`}</b></div>{!done?<div className="cards">{options.map(p=><Card key={p.id} p={p} attribute={game==="player"?A[round]:undefined} onPick={()=>pick(p)}/>)}</div>:<div className="ready"><div className="picked-strip">{picks.map(p=><div key={p.slot}><span>{p.slot}</span><b>{game==="player"?p.value:overall(p)}</b><small>{p.name}</small></div>)}</div><h2>{game==="perfect"?"CAN THEY WIN THEM ALL?":game==="era"?"YOUR XI IS READY.":"THE PLAYER IS BUILT."}</h2><button className="primary" onClick={run}>SIMULATE THE SEASON <span>↗</span></button></div>}</section>}
