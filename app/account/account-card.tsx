...
export function AccountCard({onClose}:{onClose:()=>void}) {
  ...
  const loadRecord=useCallback(async(userId:string)=>{
    ...
    setProfile(p.data);setNameDraft(p.data?.display_name||"");setStats(s.data as Stats|null);setRuns((r.data||[]) as Run[]);setAchievementCount(a.count||0);setLeaderboardCount(l.count||0);setStatus("");setAccountState("ready");
  },[]);
  ...
  }catch(error){if(!active)return;setStatus(error instanceof Error?error.message:"ACCOUNT DATA IS TEMPORARILY UNAVAILABLE.");setAccountState("error")}})();return()=>{active=false}},[loadRecord]);

  async function continueAsGuest(){setAuthBusy(true);setStatus("CREATING YOUR PLAYER RECORD...");try{const supabase=createSupabaseBrowserClient(),{data,error}=await supabase.auth.signInAnonymously();if(error)throw error;if(!data.user)throw new Error("Could not create the guest player.");setUser(data.user);await loadRecord(data.user.id)}catch(error){setStatus(error instanceof Error?error.message:"GUEST ACCESS FAILED.");setAccountState("error")}finally{setAuthBusy(false)}}
  async function googleAccess(){setAuthBusy(true);setStatus("OPENING GOOGLE...");try{const supabase=createSupabaseBrowserClient(),redirectTo=`${window.location.origin}/auth/callback?next=/`;const {error}=user?.is_anonymous?await supabase.auth.linkIdentity({provider:"google",options:{redirectTo}}):await supabase.auth.signInWithOAuth({provider:"google",options:{redirectTo}});if(error)throw error}catch(error){setStatus(error instanceof Error?error.message:"GOOGLE SIGN-IN FAILED.");setAccountState("error");setAuthBusy(false)}}
  async function emailAccess(event:FormEvent){event.preventDefault();if(!email.trim())return;setAuthBusy(true);setStatus("SENDING YOUR SECURE LINK...");try{const supabase=createSupabaseBrowserClient(),redirectTo=`${window.location.origin}/auth/callback?next=/`;const {error}=user?.is_anonymous?await supabase.auth.updateUser({email:email.trim()},{emailRedirectTo:redirectTo}):await supabase.auth.signInWithOtp({email:email.trim(),options:{emailRedirectTo:redirectTo}});if(error)throw error;setStatus("CHECK YOUR EMAIL FOR THE FOOTBALL ARCADE LINK.");setAccountState("signed-out")}catch(error){setStatus(error instanceof Error?error.message:"EMAIL SIGN-IN FAILED.");setAccountState("error")}finally{setAuthBusy(false)}}
  async function saveName(){const clean=nameDraft.trim();if(clean.length<2||clean.length>24){setNameStatus("USE 2–24 CHARACTERS");return}const supabase=createSupabaseBrowserClient(),{data:{user:currentUser}}=await supabase.auth.getUser();if(!currentUser)return;const {error}=await supabase.from("profiles").update({display_name:clean}).eq("id",currentUser.id);if(error){setNameStatus(error.message);return}setProfile(current=>current?{...current,display_name:clean}:current);setEditing(false);setNameStatus("NAME UPDATED")}
  ...
  return <div className="arcade-modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><section className="account-record-card modal-panel"><button className="modal-close" onClick={onClose}>CLOSE ×</button><header><span>LIFETIME STATS</span><div className="account-avatar">{(profile?.display_name||"P").slice(0,1).toUpperCase()}</div>{editing?<div className="account-name-editor"><input maxLength={24} value={nameDraft} onChange={event=>setNameDraft(event.target.value)} autoFocus/><button onClick={saveName}>SAVE NAME</button><button onClick={()=>setEditing(false)}>CANCEL</button></div>:<><h1>{profile?.display_name||"FOOTBALL ARCADE PLAYER"}</h1>{accountState==="ready"&&<button className="edit-player-name" onClick={()=>setEditing(true)}>EDIT PLAYER NAME</button>}</>}<p>@{profile?.username||"anonymous"}{nameStatus&&` · ${nameStatus}`}</p></header>{accountState==="loading"?<div className="account-loading">{status}</div>:showAccess?<section className="account-access"><span>FOOTBALL ARCADE ID</span><h2>{accountState==="error"?"CONNECTION INTERRUPTED.":"SAVE YOUR LEGACY."}</h2><p>{status||"Sign in to carry your runs, achievements and leaderboard entries across devices."}</p><div className="account-access-actions"><button disabled={authBusy} onClick={continueAsGuest}>PLAY AS GUEST</button><button disabled={authBusy} onClick={googleAccess}>CONTINUE WITH GOOGLE ↗</button></div><form onSubmit={emailAccess}><label htmlFor="modal-account-email">EMAIL MAGIC LINK</label><div><input id="modal-account-email" type="email" value={email} onChange={event=>setEmail(event.target.value)} placeholder="you@example.com" required/><button disabled={authBusy} type="submit">SEND LINK ↗</button></div></form></section>:<><div className="account-stat-grid"><article><small>DRAFTS COMPLETED</small><b>{stats?.verified_runs||runs.length}</b></article><article><small>ALL-TIME RECORD</small><b>{totals.wins}-{totals.draws}-{totals.losses}</b><em>{winRate}% win rate</em></article><article><small>CHAMPIONSHIPS</small><b>{stats?.league_titles||0}</b></article><article><small>BEST SCORE</small><b>{bestRun?.score||stats?.best_score||0}</b></article><article><small>FAVORITE ERA</small><b>{favoriteEra.toUpperCase()}</b><em>{favoriteEraCount} played</em></article><article><small>ACHIEVEMENTS</small><b>{achievementCount}</b></article><article><small>TOP 100 ENTRIES</small><b>{leaderboardCount}</b></article><article><small>INVINCIBLE SEASONS</small><b>{stats?.invincible_seasons||0}</b></article></div><section className="account-favorites"><article><small>MOST DRAFTED PLAYER</small><b>{favoritePlayer}</b><em>{favoritePlayerCount}× selected</em></article><article><small>MOST SELECTED COACH</small><b>{favoriteCoach}</b><em>{favoriteCoachCount}× appointed</em></article><article><small>MOST PLAYED LEAGUE</small><b>{favoriteLeague}</b><em>{favoriteLeagueCount}× entered</em></article><article><small>MOST PLAYED ERA</small><b>{favoriteEra.toUpperCase()}</b><em>{favoriteEraCount}× entered</em></article></section><section className="era-records"><span>RECORD BY ERA</span>{eraRows.map(row=><article key={row.era}><b>{row.era.toUpperCase()}</b><div><strong>{row.w}-{row.d}-{row.l} <i>({row.winRate}%)</i></strong><small>{row.played} verified draft{row.played===1?"":"s"}</small></div><em>CHAMPIONSHIPS {row.titles}×</em></article>)}</section></>}</section></div>
}
